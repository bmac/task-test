...
export const dropQueuedTasksPolicy = {
  ...
  schedule(scheduler) {
    ...
    // [a,b,c] []
    saturateActiveQueue(scheduler);
    scheduler.spliceTaskInstances(
      this.cancelReason,
      scheduler.queuedTaskInstances,
      0,
      scheduler.queuedTaskInstances.length
    );
  },
...
export const cancelOngoingTasksPolicy = {
  ...
  schedule(scheduler) {
    ...
    let numToShift = Math.max(
      0,
      activeTaskInstances.length - scheduler.maxConcurrency
    );
    scheduler.spliceTaskInstances(
      this.cancelReason,
      activeTaskInstances,
...
export const dropButKeepLatestPolicy = {
  ...
  schedule(scheduler) {
    ...
    scheduler.spliceTaskInstances(
      ...
      scheduler.queuedTaskInstances,
      0,
      scheduler.queuedTaskInstances.length - 1
    );
  }
};
